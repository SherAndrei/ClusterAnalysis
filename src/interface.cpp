...
static void get_param(istream& is, bool& ok, double& param)
{
    if(ok) {
        if(is.peek() != ',') {
            ok = ok && is >> param;
...
void Interface::get_command(istream& is)
{
    ...
    while(is >> command)
    {
        if(command == "CREATE") {
            ...
            controller.cluster(meanX, meanY, varianceX, varianceY, N);
			...
			bool ok = true;
			if(!(ok && (is >> delta))) {
                cout << "Wrong format: " << command << ' ' << delta << '\n';
                log_in("Wrong format: " + command + ' ' + to_string(delta));
                continue;
			}
			log_in(command + ' '  + to_string(delta));
